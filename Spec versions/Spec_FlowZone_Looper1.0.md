...
**Version:** 4.0 (Added Settings Panel Specification) **Target Framework:** JUCE 8 (C++20 standard) \+ React 18 **Platform:** macOS (Standalone App) **Repository Structure:** Monorepo (Engine \[C++\] / WebClient \[React/TS\] / Shared)
...
* **Hybrid Web Architecture:** The application core is a native C++ audio engine, but the User Interface is a **React** web application.  
  ...
  * **Remote Control:** The app hosts an internal web server (port 8080), allowing other devices on the LAN (iPads, phones, laptops) to load the exact same UI and control the session.  
  * **Collaborative Shared Session:** All connected users (local and remote) control the **same single audio session**.  
...
* **`WebServer`:** Embedded HTTP/WebSocket server.  
* **`WebUIContainer`:** `juce::WebBrowserComponent`.  
* **`SandboxManager`:** Manages VST3 child processes.
...
* **Command Loop:** React \-\> JSON \-\> WebSocket \-\> C++ Engine.  
* **Metering Loop:** C++ Engine \-\> Binary WebSocket \-\> React Canvas.  
...
* **Network Disconnect:** UI overlay "Reconnecting...". Engine continues.  
* **Sandbox Crashes:** Toast notification. Restart worker request.  
...
3. **Task 1.3:** Implement `APIBridge`.  
4. **Task 1.4:** Implement `DiskWriter` using `juce::TimeSliceThread`.
