...
**Version:** 1.5 (Protocol Precision & Operational Depth)  
...
*   **Premium Visualization:** 
    ...
    *   **Adaptive Degradation:** Stepped CPU-aware throttling (see §3.4 for thresholds).
*   **Operational Robustness:**
    *   **Crash Guard:** Named component that detects boot loops and enforces Safe Mode (see §2.2.F).
...
*   **Role:** Detects boot loops and enforces Safe Mode at startup.
*   **Mechanism:**
    1.  On launch, reads crash counter from `~/Library/Application Support/FlowZone/.crash_sentinel`.
    2.  Increments counter immediately on boot.
    3.  Clears counter after **60 seconds** of stable runtime.
    4.  If counter ≥ 3 on launch → enters `SAFE_MODE`.
*   **Safe Mode behavior:** All VSTs disabled, default audio device, no auto-load of last session.
...
*   **Rate:** Target 30fps (subject to CPU-aware degradation per §3.4).
...
### **3.5. Binary Visualization Stream**
...
### **3.6. Adaptive Visualization Degradation**
...
| **Boot Loop** | Critical | CrashGuard counter ≥ 3 | Enter **SAFE MODE**. | Disable VSTs, default audio, no auto-load. |
