...
**UI Architecture:** JUCE Native C++ (WebSockets + React Web UI deferred to V2 remote control)
...
*   **Identity:** A macOS-first retrospective looping workstation named **Samsara**.
...
*   **Native Architecture:** C++ Audio Engine (Source of Truth) + JUCE Native UI.
    *   **Local:** JUCE GUI components (Sliders, Custom Canvas Visualizers).
    *   **Remote (V2):** Internal WebServer using CivetWeb for phone/tablet control over LAN via React Web UI.
...
*   **Mechanism:**
    1.  On launch, reads crash counter and context from `~/Library/Application Support/Samsara/.crash_sentinel`.
...
*   **Implementation:**
    ...
    *   **Auto-save:** Writes a recovery snapshot every 30 seconds to `~/Library/Application Support/Samsara/backups/autosave.json`.
...
*   **Connection Lifecycle:**
    ...
    8.  **Production Content Serving:** CivetWeb serves the Vite build output from the local filesystem (`~/Library/Application Support/Samsara/web_client/` or bundled alongside the app). In development, `WebBrowserComponent::goToURL("http://localhost:5173")` enables Vite HMR. The app detects the environment and chooses the appropriate URL. There is no `ResourceProvider` fallback; strict separation of concerns validation.
...
Samsara uses **always-on capture** — there is no explicit "record start" or "record stop" command. The audio engine continuously captures all played audio into a fixed-size rolling retrospective buffer.
...
*   **Session Metadata:** `~/Library/Application Support/Samsara/sessions/[session_id]/session.json`
    *   **Content:** Full `AppState` snapshot (JSON) excluding transient fields (system metrics, barPhase).
*   **Riff Audio:** `~/Library/Application Support/Samsara/sessions/[session_id]/audio/riff_{timestamp}_slot_{index}.flac`
*   **Autosave:** `~/Library/Application Support/Samsara/backups/autosave.json` (Full AppState snapshot).
*   **Configuration:** `~/Library/Application Support/Samsara/config.json`. Application-level settings (Audio Device, VST Paths). Not per-session.
...
*   **Storage:** `~/Library/Logs/Samsara/engine.jsonl`
...
**Decision:** Projucer (`Samsara.jucer`) is the authoritative build system. It generates the Xcode project for the main application.
...
- Every bead that adds C++ source files must add them to the Projucer project (`Samsara.jucer`).
...
├── Projucer project configured (Samsara.jucer)
...
Samsara does not need exhaustive test coverage for V1. Instead, testing is focused on the **dangerous boundaries** — where C++ meets TypeScript, where the audio thread meets the message thread, and where components integrate. Each bead must include its own verification, but the scope is kept practical.
...
|:---|:---|:---|:---|
| **C++ (Engine)** | [Catch2](https://github.com/catchorg/Catch2) v3 | Add as separate Projucer Console Application or Xcode test target. Test binary: `SamsaraTests` | `./build/Debug/SamsaraTests` |
...
| Catch2 test passes | Run `SamsaraTests` |
| Vitest test passes | Run `npm test` in `web_client/` |

> **What you should see:** A dark grey window opens (approximately 800×600 pixels). In the center, white text reads "Hello Samsara" (or similar React placeholder). No crash dialogs appear. In Xcode's console output at the bottom, you should see a line like `[WebSocket] Server listening on port 8765` followed by `[WebSocket] Client connected`. There should be **no red error text** in the Xcode console. If you open Terminal and run the test commands, both should print green "PASSED" or "✓" with zero failures.
...
| All Catch2 + Vitest tests pass | `SamsaraTests` + `npm test` |
...
1.  **Task 0.1:** Set up Projucer project (`Samsara.jucer`) with aggressive compiler warnings (`-Wall -Wextra`). Integrate **CivetWeb** as source files in a `libs/civetweb/` directory added to Projucer as a source group. Configure **Catch2** (as a separate Projucer Console Application or Xcode test target) and **Vitest** (in `web_client/`).
